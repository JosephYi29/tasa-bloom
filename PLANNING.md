...
- [x] **Board Position Admin Toggle** (`/protected/admin/settings/positions`): Admins can toggle which positions grant admin access via an "Admin" toggle switch on each position row. Added "Advisor" position with admin privileges (migration `23_add_advisor_position.sql`). New `togglePositionAdmin` server action updates `is_admin` on any position.
- [x] **Admin Check Bug Fix** (`lib/authUtils.ts`): Fixed `getCurrentUser()` failing to detect admin status for non-super-admin users. The Supabase query tried to join `profiles` through `board_memberships` without a direct foreign key, causing PostgREST to fail silently and return null — making `isAdmin` always false. Split into two separate queries: one for `board_positions` (via FK) and one for `profiles` (direct lookup).
...
- [x] **Global Scoring Cache**: Implemented Next.js `unstable_cache` and `revalidateTag` to cache heavy scoring computations globally, reducing Results page latency by ~81%.
- [ ] Loading states and skeleton screens
---
